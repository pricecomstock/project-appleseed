class PointTracker {
  ...
  distributePoints(votingResults, currentVotingMatchup) {
    ...
    const pointsPerVote = Math.round(this._pointsPerPrompt / numVotes);
    ...
    const pointsArray = voteCountArray.map((ct) => ct * pointsPerVote);
    ...
    currentVotingMatchup.answers.forEach((answerPair, index) => {
      ...
      if (isShutout && index === shutoutIndex) {
        pointsForAnswer += this._pointsForShutout;
      }
    ...
    const scoringDetails = {
      ...
      isShutout,
      shutoutIndex,
      shutoutPoints: this._pointsForShutout,
    };
